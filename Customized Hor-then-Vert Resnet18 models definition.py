# Customized Hor-then-Vert Resnet18 models definition
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, i, Layer, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if Layer == "1":
            if i == 1:
                self.conv1 = conv1x9(inplanes, planes, stride=(1,2))
            else:
                self.conv1 = conv1x9(inplanes, planes, stride=(1,1))
            self.conv2 = conv1x7(planes, planes)
        elif Layer == "2":
            if i == 1:
                self.conv1 = conv1x7(inplanes, planes, stride=(1,2))
            else:
                self.conv1 = conv1x7(inplanes, planes, stride=(1,1))
            self.conv2 = conv1x5(planes, planes)
        elif Layer == "3":
            if i == 1:
                self.conv1 = conv1x5(inplanes, planes, stride=(1,2))
            else:
                self.conv1 = conv1x5(inplanes, planes, stride=(1,1))
            self.conv2 = conv5x1(planes, planes)
        elif Layer == "4":
            if i == 1:
                self.conv1 = conv5x1(inplanes, planes, stride=(2,1))
            else:
                self.conv1 = conv5x1(inplanes, planes, stride=(1,1))
...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, num_classes=3, zero_init_residual=False,
...
def resnet10(pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet10', BasicBlock, [2, 2, 0, 0], pretrained, progress,
                   **kwargs)
